  ...
  config = mkIf cfg.enable {
    ...
    khome.tuigreet.sessions = {
      hyprland = {
        ...
        # start-hyprland is Hyprland's own watchdog binary: it execs
        # Hyprland and restarts it if it dies non-cleanly. Session-lifecycle
        # (graphical-session.target, env export) is handled separately by
        # home-manager's wayland.windowManager.hyprland.systemd config
        # (hyprland/default.nix), not by this command.
