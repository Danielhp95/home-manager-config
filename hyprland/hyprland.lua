...
-- ─────────────────────────────────────────────────────────────────────────────
-- Environment variables (Nvidia multi-monitor, see wiki.hypr.land/Nvidia)
-- ─────────────────────────────────────────────────────────────────────────────
...
hl.on("hyprland.start", function()
    ...
    hl.exec_cmd("dbus-update-activation-environment --systemd DISPLAY HYPRLAND_INSTANCE_SIGNATURE WAYLAND_DISPLAY XDG_CURRENT_DESKTOP QT_QPA_PLATFORMTHEME")
...
hl.bind(mod .. " + SHIFT + B",    hl.dsp.exec_cmd("bash /home/dani/nix_config/menu_launchers/scripts/choose_bluetooth_device_from_paired.sh"))
...
hl.bind(mod .. " + SHIFT + N",    hl.dsp.exec_cmd("swaync-client --open-panel"))
hl.bind(mod .. " + SHIFT + CONTROL + H", hl.dsp.exec_cmd("swaync-client --toggle-dnd")) -- "H" as in hush
-- NOTE: in the original conf, SUPER+SHIFT+R and SUPER+SHIFT+S each had two binds;
-- the later one wins. Reproduced faithfully below.
...
hl.bind(mod .. " + SHIFT + S",    hl.dsp.exec_cmd("hyprshot --mode=region --raw --clipboard-only | satty -f - --copy-command wl-copy --early-exit"))
hl.bind(mod .. " + SHIFT + A",    hl.dsp.exec_cmd("pavucontrol"))
hl.bind(mod .. " + CONTROL + SHIFT + L", hl.dsp.exec_cmd("hyprlock"))

-- Toggle bar
hl.bind(mod .. " + m", hl.dsp.exec_cmd("noctalia msg bar-toggle"))

-- Normal workspaces
hl.bind(mod .. " + Space", hl.dsp.window.float({ action = "toggle" }))
hl.bind(mod .. " + s",     hl.dsp.window.toggle_swallow())
...
-- Focus (hy3)
hl.bind(mod .. " + h",     hy3.move_focus("left"))
hl.bind(mod .. " + j",     hy3.move_focus("down"))
hl.bind(mod .. " + k",     hy3.move_focus("up"))
hl.bind(mod .. " + l",     hy3.move_focus("right"))
hl.bind(mod .. " + left",  hy3.move_focus("left"))
hl.bind(mod .. " + right", hy3.move_focus("right"))
hl.bind(mod .. " + down",  hy3.move_focus("down"))
hl.bind(mod .. " + up",    hy3.move_focus("up"))

-- Move window (hy3)
hl.bind(mod .. " + SHIFT + h", hy3.move_window("left"))
hl.bind(mod .. " + SHIFT + j", hy3.move_window("down"))
hl.bind(mod .. " + SHIFT + k", hy3.move_window("up"))
hl.bind(mod .. " + SHIFT + l", hy3.move_window("right"))

-- Change focus level within a group (hy3)
hl.bind(mod .. " + SHIFT + u", hy3.change_focus("raise"))
hl.bind(mod .. " + SHIFT + d", hy3.change_focus("lower"))
hl.bind(mod .. " + SHIFT + r", hy3.change_focus("bottom"))

hl.bind(mod .. " + SHIFT + Space", hy3.toggle_focus_layer())
hl.bind(mod .. " + C",             hl.dsp.window.center())

hl.bind(mod .. " + s", hy3.set_swallow("toggle"))             -- duplicate of SUPER+s above (faithful)
...
-- Move submap (axis mapping kept identical to the original conf)
hl.bind(mod .. " + M", hl.dsp.submap("move"))
hl.define_submap("move", function()
    hl.bind("left",  hl.dsp.window.move({ x = -40, y = 0,   relative = true }), { repeating = true })
    hl.bind("up",    hl.dsp.window.move({ x = 40,  y = 0,   relative = true }), { repeating = true })
    hl.bind("down",  hl.dsp.window.move({ x = 0,   y = -40, relative = true }), { repeating = true })
    hl.bind("right", hl.dsp.window.move({ x = 0,   y = 40,  relative = true }), { repeating = true })
...
hl.bind(mod .. " + SHIFT + f", hl.dsp.exec_cmd("wl-kbptr -o modes=floating,click -o mode_floating.source=detect --config=/home/dani/.config/wl-kbptr.yaml"))
hl.bind(mod .. " + ALT + l", hl.dsp.exec_cmd("wlrctl pointer click left"),   { repeating = true })
hl.bind(mod .. " + ALT + m", hl.dsp.exec_cmd("wlrctl pointer click middle"), { repeating = true })
hl.bind(mod .. " + ALT + r", hl.dsp.exec_cmd("wlrctl pointer click right"),  { repeating = true })
-- Scroll up / down (overrides the SHIFT+u/d hy3 changefocus binds above, as in the conf)
hl.bind(mod .. " + SHIFT + u", hl.dsp.exec_cmd("wlrctl pointer scroll +20 +0"), { repeating = true })
hl.bind(mod .. " + SHIFT + d", hl.dsp.exec_cmd("wlrctl pointer scroll -20 +0"), { repeating = true })
...
hl.window_rule({ name = "firefox-ws1",   match = { class = "firefox" },                                          workspace = 1 })
hl.window_rule({ name = "steam-ws6",     match = { class = "steam" },                                            workspace = 6 })
hl.window_rule({ name = "spotify-ws8",   match = { title = "Spotify" },                                          workspace = 8 })
hl.window_rule({ name = "messaging-apps", match = { class = "^(Slack|org\\.telegram\\.desktop|Element)$" },      workspace = 9 })

-- Floating windows
hl.window_rule({ name = "float-general-title", match = { title = "^(Weather|MainPicker)$" }, float = true })
hl.window_rule({ name = "float-general",       match = { class = "^(io\\.github\\.amit9838\\.mousam|Rofi|org\\.pulseaudio\\.pavucontrol|blueberry|mpv|imv|satty)$" }, float = true })
...
hl.window_rule({ name = "pavucontrol", match = { class = "org.pulseaudio.pavucontrol" }, size = "1200 900", float = true, center = true })
hl.window_rule({ name = "tile-grayjay", match = { title = "GrayJay" }, tile = true })

-- Opacity
hl.window_rule({ name = "opacity-multiclass",       match = { class = "^(pavucontrol|zoom|firefox|satty|mpv|matplotlib)$" }, opacity = "1.0 override 1.0 override" })
hl.window_rule({ name = "opacity-kitty-fullscreen", match = { class = "kitty", fullscreen = 1 },                            opacity = "0.9 override 0.9 override" })
hl.window_rule({ name = "opacity-weather",          match = { title = "Weather" },                                          opacity = "1.0 override 1.0 override" })
hl.window_rule({ name = "opacity-zoom-initial",     match = { initial_title = "zoom" },                                     opacity = "1.0 override 1.0 override" })
